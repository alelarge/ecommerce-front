...
export const cartSlice = createSlice({
  ...
  reducers: {
    setProductQuantity: (state, action: PayloadAction<{ productId: number; quantity: number }>) => {
      ...
      const productIndex = getProductIndexFromCart(state.products, productId);
      if (productIndex !== -1) {
    ...
    substract: (state, action: PayloadAction<ProductType>) => {
      const productToRemove = action.payload;
      const productIndex = getProductIndexFromCart(state.products, productToRemove.id);
      if (productIndex !== -1) {
...
export const { add, substract, remove, setProductQuantity } = cartSlice.actions;
